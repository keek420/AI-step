...
#エラーログをdbにコミット
def commit_error_msg(db_user, error_msg:str):
	#TODO
	db_user.ans = "0.000"
	db_user.is_error = True
...
def send_mail(name:str, mail:str, csv:str="", code:str="", available_upload_num:int=4 ,is_correct_receive:bool=True, sent_error_msg:str = "" ):
	...
	try:
		...
		stat = server.ehlo("mylowercasehost")
		#print(stat)
		server.starttls()
...
def upload(user_upload_data: str):
	...
	if db_user is None:
			...
			db_user = db(
				...
				upload_date = datetime.fromtimestamp(int(data["uploadTime"])).strftime("%Y-%m-%d %H:%M:%S"),
				upload_num = 0,
			)
			session.add(db_user)	
			session.commit()
	...
	# アップロード時間から再提出したのか判定。
		...
		if (int(data["uploadTime"]) <= db_user.upload_unix):
			print("再提出はされていません")
			print(f'moodle upload_time :{int(data["uploadTime"])}')
			print(f'db upload_time     :{db_user.upload_unix}')
			session.close()
			return 1
	...
	#提出回数が5回まで
	if db_user.upload_num >=  MAX_UPLOAD_NUM:
		error_msg = "一日の提出上限回数を超えました。\n今日はこれ以上提出できません。"
		db_user.is_error = True
		db_user.error_msg = error_msg
		session.commit()
	...
	db_user.upload_unix = int(data["uploadTime"])
	db_user.upload_date = datetime.fromtimestamp(int(data["uploadTime"])).strftime("%Y-%m-%d %H:%M:%S")


	if os.path.exists(tmp_csv):

		print(f"tmp_csv {tmp_csv}")	
		return
	# コマンド作成
	cmd_list = []
	...
	try:
		...
		print(str(e))
		commit_error_msg(db_user, error_msg)
		return send_mail(name, data["email"], is_correct_receive=False, sent_error_msg=error_msg)
	...
	if(err_str != ""):
		...
		if "行目の列数が2以外です。" in err_str or "全データのフォーマットが誤りです。" in err_str:
			commit_error_msg(db_user, err_str)
			print(err_str)
	...
	db_user.upload_unix = int(data["uploadTime"])
	db_user.upload_date = datetime.fromtimestamp(int(data["uploadTime"])).strftime("%Y-%m-%d %H:%M:%S")
	db_user.is_error = False
	db_user.error_msg = ""
	db_user.upload_num += 1

	# unixをJSTの文字列に変換
	session.commit()

	result_db = session.query(db).filter(db.id == uid).one_or_none()
	...
	# 控えメールの送信
	mail_res = send_mail(result_db.nickname, result_db.mail, csv=result_db.csv, code=result_db.code, available_upload_num= MAX_UPLOAD_NUM - db_user.upload_num )
